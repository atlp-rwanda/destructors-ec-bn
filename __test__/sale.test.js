...
describe('sallerUpdateSaleStatus', () => {
    let order,newseller,sellerToken,products, Sale ,createdOrder, user;
...
const createdSale = await Sales.findOne({where:{id:'128a0571-4538-4de1-9ed5-62151d16eb6c'}})
            ...
            order = await Orders.create({
              ...
              userId: 'd0db925d-03b7-4e7a-a838-7a8d0823fd97',
...
describe('getsles', () => {
  ...
  let order,newseller,sellerToken, products;
  it('should return 401 if the sale is  not related to the current seller', async () => {
  ...
  let Sale ;
  let user
    it('should return 404 once the seller has no sales yet', async () => {
const createdSale = await Sales.findOne({where:{id:'128a0571-4538-4de1-9ed5-62151d16eb6c'}})
        ...
        it('should return all sales when logged in user is admin', async ()=>{
        ...
        expect(response.status).toBe(200);
                ...
                it('should return all sales for the logged in seller', async ()=>{
                  const user = await User.findOne({ where: { email: 'seed@gmail.com' } });
                  sellerToken = generateToken(user);
                  const response = await request(app)
                  .get('/api/v1/sales')
