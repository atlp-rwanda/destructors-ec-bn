...
const changeSaleStatus = async (req, res) => {
  try {
    ...
    await Sales.update({ status: newStatus }, { where: { id: req.params.id } });
      ...
      for (const orderSellerId of orderSellerIds) {
        for (const orderProductName of orderProductNames) {
          const currentProduct = await Products.findOne({
            where: { sellerId: orderSellerId, name: orderProductName },
          });

          const orderPquantity = orderProducts.find(
            (data) => data.sellerId === orderSellerId
          ).quantity;
          const newQuantity = currentProduct.quantity - orderPquantity;
          await Products.update(
            { quantity: newQuantity },
            { where: { name: orderProductName } }
          );
        }
